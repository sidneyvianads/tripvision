...
export default function Layout({ tabLabel, children }) {
  ...
  const initial = (profile?.nome ?? user?.email ?? "?").trim().charAt(0).toUpperCase();
  const displayName = profile?.nome ?? user?.user_metadata?.full_name ?? user?.email ?? "Viajante";
      ...
      <header className="gradient-header text-white safe-top">
        <div className="px-4 pt-4 pb-5 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-white/80 text-xs truncate">Olá, {displayName.split(" ")[0]}!</div>
          </div>
          <button
